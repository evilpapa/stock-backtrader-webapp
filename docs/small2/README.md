...
使用 `build_small2_snapshots()` 在每个调仓日构建 `StockUniverseSnapshot`；将这些快照传给 `run_small2_backtest()`。`Small2Strategy` 只会选择当前日期或更早的最近快照，避免使用未来的 ST、上市状态、流通股本和市值。

`prepare_small2_price_data()` 需要拉取快照覆盖股票与中证 1000 (`000852.SH`)、沪深 300 (`000300.SH`) 的日线，并保留 QMT 的 `amount` 字段。

## 重要限制

- 全市场每日请求量较大。应将 `build_small2_snapshots()` 的产物在本地缓存为 Parquet/CSV，而非对每次页面刷新重抓。
