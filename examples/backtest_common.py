...
def prepare_price_data(
	...
	for symbol in symbols:
		...
		if duplicate_count:
			print(f"  WARN {symbol}: 已合并 {duplicate_count} 条重复日期记录")
			df = df.loc[~df.index.duplicated(keep="last")]
		valid_price = df[["Open", "High", "Low", "Close"]].gt(0).all(axis=1)
		invalid_count = int((~valid_price).sum())
		if invalid_count:
			print(f"  WARN {symbol}: 已剔除 {invalid_count} 条 OHLC 非正值记录")
			df = df.loc[valid_price]
		if df.empty:
...
def align_series(*series: pd.Series) -> list[pd.Series]:
	...
	# 最后一条代表该交易日的日终收益。
	normalized = [current.groupby(level=0).last() for current in series]
	common_index = normalized[0].index
	for current in normalized[1:]:
		common_index = common_index.intersection(current.index)
	return [current.reindex(common_index).dropna() for current in normalized]
...
def plot_weights(weights_df: pd.DataFrame, strategy_name: str, output_dir: Path) -> None:
	...
	output_dir.mkdir(parents=True, exist_ok=True)
	fig.savefig(output_dir / "daily_weights_plot.png", dpi=250, format="png")
	plt.close(fig)
