...
OUTPUT_DIR = project_path("examples", "etf_momentum")
...
def parse_data_source_params(argv: Sequence[str] | None = None) -> DataSourceParams:
	"""Read Big QMT RPC configuration for this standalone example."""
	parser = argparse.ArgumentParser(description="ETF 动量轮动策略回测")
	parser.add_argument(
		"--bigqmt-account-id",
		default=os.getenv("BIGQMT_ACCOUNT_ID", ""),
		help="Big QMT 资金账号（默认读取 BIGQMT_ACCOUNT_ID）",
	)
	parser.add_argument(
		"--bigqmt-timeout",
		type=float,
		default=float(os.getenv("BIGQMT_RPC_TIMEOUT_SECONDS", "10")),
		help="Big QMT RPC 请求超时（秒）",
	)
	args = parser.parse_args(argv)
	return DataSourceParams(
		bigqmt_account_id=args.bigqmt_account_id,
		bigqmt_timeout=args.bigqmt_timeout,
	)
...
def main() -> None:
	...
	print("数据源: xtquant-big-convert Big QMT RPC")
	print("=" * 60)
