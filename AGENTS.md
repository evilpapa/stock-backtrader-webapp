...
- `charts/`：图表渲染。
- `utils/`：数据处理、日志、schemas、QMT API 客户端等工具。
- `tests/`：测试用例。
...
- 普通 Python 代码通过 `utils.qmt_client.fetch_history_ohlcv` 访问 QMT Python API。
...
2. `utils/processing.gen_stock_df` 通过 QMT API 获取数据并返回精简 DataFrame。
