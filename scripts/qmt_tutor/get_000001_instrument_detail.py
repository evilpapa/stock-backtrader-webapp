...
def fetch_instrument_detail(
    ...
    *,
    ...
    """通过 ``xtquant_compat.xtdata.get_instrument_detail`` 获取合约概况。"""
    ...
    try:
        detail = xtdata.get_instrument_detail(qmt_symbol)
    except Exception as exc:
        raise BigQmtApiError(f"Big QMT get_instrument_detail 请求失败: {exc}") from exc
    if not isinstance(detail, dict):
        raise BigQmtApiError(f"未找到合约概况: {qmt_symbol}")
    return detail
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--brief", action="store_true", help="只请求基础合约字段，即 iscomplete=False")
