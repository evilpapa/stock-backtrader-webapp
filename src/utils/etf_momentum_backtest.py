...
DEFAULT_OUTPUT_DIR = Path("examples") / "etf_momentum"
...
def load_etf_momentum_results(output_dir: Path = DEFAULT_OUTPUT_DIR) -> EtfMomentumFrames | None:
	"""从输出目录读取已生成的 ETF 动量回测结果；文件不完整时返回空。"""
...
def run_etf_momentum_backtest(
	...
	strategy_result = run_strategy_backtest(
		...
		available_symbols,
		available_names,
		strategy_class or EtfMomentumStrategy,
		STRATEGY_NAME,
		initial_cash,
		strategy_params or {
