...
class RotationStrategyBase(BaseStrategy):
	...
	params = (
		("momentum_window", 20),  # 风险调整动量计算窗口
		("rebalance_days", 5),  # 调仓间隔（交易日）
		("top_l", 5),  # 入选标的数量
		("benchmark_index", None),  # 基准数据源在 self.datas 中的索引，不参与交易
	...
	def __init__(self):
		...
		for data in self.datas:
			...
			self.momentum.append(
				bt.indicators.SimpleMovingAverage(return_series, period=self.p.momentum_window)
			)
			self.volatility.append(
				bt.indicators.StandardDeviation(return_series, period=self.p.momentum_window)
			)

	def next(self):
		"""按设定频率计算目标权重，并执行一次组合再平衡。"""
		if self._has_pending_orders() or any(len(data) < self.p.momentum_window for data in self.datas):
			return
	...
	def _build_target_weights(self) -> tuple[np.ndarray, list[int], dict[str, float], dict[str, float]]:
		...
		selected = candidates[: self.p.top_l]
	...
	def _get_tradable_indices(self) -> list[int]:
		...
		benchmark_index = self.p.benchmark_index
		return [
			index
