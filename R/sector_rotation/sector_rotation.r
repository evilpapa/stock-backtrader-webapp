...
colnames(cumulative_all) <- c("行业动量轮动策略", benchmark_name, "等权重组合")
...
colnames(drawdown_all) <- c("行业动量轮动策略", benchmark_name, "等权重组合")
...
p1 <- create_combined_plot(
  ...
  "行业动量轮动策略", "科创50ETF",
  sprintf("行业动量轮动策略 vs 科创50ETF (N=%d, K=%d, L=%d)", N, K, L),
  colors_vs_benchmark
)
...
p2 <- create_combined_plot(
  cumulative_df, drawdown_df,
  "行业动量轮动策略", "等权重组合",
  sprintf("行业动量轮动策略 vs 等权重组合 (N=%d, K=%d, L=%d)", N, K, L),
  colors_vs_equal
)
...
# 计算各策略绩效
strategy_perf <- calculate_performance(strategy_returns, "行业动量轮动策略")
...
# ==================== 保存结果到文件 ====================
cat("正在保存结果到文件...\n")
output_dir <- sprintf("行业动量轮动策略_N%d_K%d_L%d", N, K, L)
