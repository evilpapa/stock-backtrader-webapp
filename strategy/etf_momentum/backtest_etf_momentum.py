...
def main():
    ...
    if len(data_dict) < len(ETF_SYMBOLS):
        print("\n警告: 部分ETF数据获取失败，回测可能不完整")
    
    # 2. 运行动量策略回测
    cerebro, strategy_result = run_backtest(data_dict, ETF_SYMBOLS, ETF_NAMES)
    
    # 3. 运行基准策略回测（沪深300）
    benchmark_result = run_benchmark_backtest(
        data_dict, "510300.SS", "沪深300ETF"
    )
    
    # 4. 运行等权重组合回测
    equal_weight_result = run_equal_weight_backtest(
        data_dict, ETF_SYMBOLS, ETF_NAMES
    )
